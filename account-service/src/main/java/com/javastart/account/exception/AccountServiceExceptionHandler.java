...
public class AccountServiceExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        return new ResponseEntity<>(new TemplateAccountList(fields, OffsetDateTime.now(),
                errors), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler({ConstraintViolationException.class})
    protected ResponseEntity<TemplateAccount> handleConstraintViolationException(ConstraintViolationException ex) {
        return new ResponseEntity<>(new TemplateAccount(ex.getMessage()
                .substring(ex.getMessage().indexOf(" ") + 1), OffsetDateTime.now(),
                ex.getClass().getSimpleName()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler({AccountNotFoundException.class})
    public ResponseEntity<TemplateAccount> handleAccountNotFoundException(AccountNotFoundException ex) {
        return new ResponseEntity<>(new TemplateAccount(ex.getMessage(), OffsetDateTime.now(),
                ex.getClass().getSimpleName()), HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler({CreateBillException.class})
    public ResponseEntity<TemplateAccount> handleAccountNotFoundException(CreateBillException ex) {
        return new ResponseEntity<>(new TemplateAccount(ex.getMessage(), OffsetDateTime.now(),
                ex.getClass().getSimpleName()), HttpStatus.BAD_REQUEST);
    }
}
